...
% Eliminate short track (optional)
...
for t=1:length(est.T)
    ...
    for u=2:length(tah)
        [m_cell{u},P_cell{u}] = kalman_predict_multiple(model,m_cell{u-1},P_cell{u-1});
        w_cell{u} = w_cell{u-1} ; 
        k= ks+u-1;
        emm= tah(u);
        if emm > 0
            [qz,m_cell{u},P_cell{u}] = kalman_update_multiple(meas.Z{k}(:,emm),model,m_cell{u},P_cell{u});
    ...
    for u= length(tah)-1 : -1 : 1
        m = rts_smooth_estimate(model,w_cell{u},m_cell{u},P_cell{u}, ...
            w_cell{u+1},m_cell{u+1}) ;
        k= ks+u-1;
        est.N(k)= est.N(k)+1;
