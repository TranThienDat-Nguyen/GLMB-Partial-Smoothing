...
% based on the original filter proposed in
...
for k=1:meas.K
    ...
    prune_flag = 0 ; 
    prune_thres = 3 ; % threshold (length) to prune short tracjectories
    if k==meas.K % only prune short trajectories at the last time step
        prune_flag = 1 ; 
    end
    est_smooth = extract_estimates_recursive_smooth(glmb_update,model,meas,est_smooth,prune_flag,prune_thres,meas.K) ;
...
function est=extract_estimates_recursive_smooth(glmb,model,meas,est,prune_flag,prune_thres,K)
...
for t=1:length(est.T)
    ...
    for u=2:length(tah)
        ...
        [m_cell{u},P_cell{u}] = ukf_predict_multiple(model,m_cell{u-1},P_cell{u-1},est.filter.ukf_alpha,est.filter.ukf_kappa,est.filter.ukf_beta);
        w_cell{u} = w_cell{u-1} ; 
        k= ks+u-1;
        emm= tah(u);
        if emm > 0
            [qz,m_cell{u},P_cell{u}] = ukf_update_multiple(meas.Z{k}(:,emm),model,m_cell{u},P_cell{u},est.filter.ukf_alpha,est.filter.ukf_kappa,est.filter.ukf_beta);
    ...
    for u= length(tah)-1 : -1 : 1
        m = urts_smooth_estimate(model,w_cell{u},m_cell{u},P_cell{u}, ...
            w_cell{u+1},m_cell{u+1},est.filter.ukf_alpha,est.filter.ukf_kappa,est.filter.ukf_beta) ;
        k= ks+u-1;
        est.N(k)= est.N(k)+1;
